...
LOD.FCT <- "Best"
## Selecting "Best" will signal the code to automatically select the best fitting model choice. Run the function getMeanFunctions() to print the list of all choices.
...
if(sum(colnames(DAT)=="Target")!=1) { #Is there a "Target" column?
  ...
  if(length(A)!=1) { write("There is a problem with the 'Target' column.\n\n",
                           file="Analysis Log.txt",
                           append=TRUE) } #Add error message to analysis log.
  if(length(A)>1) { cat("ERROR: multiple 'Target' columns detected.",
                        colnames(DAT)[A],
                        sep="\n") }
...
if(sum(colnames(DAT)=="SQ")!=1) { #Is there a "SQ" column?
  ...
  if(length(A)!=1) { write("There is a problem with the 'SQ' column.\n\n",
                           file="Analysis Log.txt",
                           append=TRUE) } #Add error message to analysis log.
  if(length(A)>1) { cat("ERROR: multiple 'SQ' columns detected.",
                        colnames(DAT)[A],
                        sep="\n") }
...
if(sum(is.na(DAT$SQ))>0) {
  write(paste0("WARNING: ",
               sum(is.na(DAT$SQ)),
               " data points excluded without a valid starting quantity (SQ)!\nHere is a sample of the data being excluded:\n"),
        file="Analysis Log.txt",
        append=TRUE)
  suppressWarnings(write.table(head(DAT[is.na(DAT$SQ),]),
                               file="Analysis Log.txt",
                               append=TRUE,
                               sep="\t",
                               eol="\n",
...
for(i in 1:length(Targets)) {
  if(i==1) {
    ...
    Target <- rep(as.character(Targets[i]),
                  length(Standards))
  }
  else {
    Standards <- c(Standards,
                   unique(DAT$SQ[DAT$Target==Targets[i]&!is.na(DAT$SQ)]))
    Target <- c(Target,
                rep(as.character(Targets[i]),
                           length(unique(DAT$SQ[DAT$Target==Targets[i]&!is.na(DAT$SQ)]))))
  }
}
OUTS <- data.frame(Target=Target,
                   Standard=Standards,
                   Outliers=NA)
...
for(i in 1:nrow(OUTS)) {
  MED <- median(DAT$Cq[DAT$SQ==OUTS$Standard[i]&DAT$Target==OUTS$Target[i]],
                na.rm=TRUE)
...
if(sum(!is.na(OUTS$Outliers))>0) {
  ...
  write.csv(DAT[OUT.ROW2,],
            file="Potential-Outliers.csv",
            row.names=FALSE)
...
## Generate standard curves using all data and calculate copy estimates for each replicate using the curves:
curve.list <- ""
DAT$Copy.Estimate <- rep(NA,
                         nrow(DAT))
DAT$Mod <- rep(0,
               nrow(DAT))
for(i in 1:length(Targets)) {
  STDS <- data.frame(S=unique(DAT$SQ[DAT$Target==Targets[i]]),
                     R=NA)
  ...
  for(j in 1:nrow(STDS)) {
    STDS$R[j] <- sum(!is.na(DAT$Cq)&DAT$SQ==STDS$S[j]&DAT$Target==Targets[i],
                     na.rm=TRUE)/sum(DAT$SQ==STDS$S[j]&DAT$Target==Targets[i],
                                     na.rm=TRUE)
  }
  ## Only use standards with 100% or greater detection rates for linear regression:
  if(sum(STDS$R>=1,
         na.rm=TRUE)>2) {
    STDS2 <- STDS$S[STDS$R>=1&!is.na(STDS$R)&!is.na(STDS$S)]
  }
  ## If there are not at least 3 standards with 100% or greater detection, use the top 3:
  if(sum(STDS$R>=1,
         na.rm=TRUE)<3) {
    STDS2 <- STDS$S[order(STDS$R,
                          decreasing=TRUE)][1:3]
  }
  ...
  for(j in 1:length(STDS2)) {
    ...
    DAT$Mod[DAT$Target==Targets[i]&DAT$SQ==STDS2[j]&DAT$Cq>=quantile(D,
                                                                     na.rm=TRUE)[2]&DAT$Cq<=quantile(D,
                                                                                                     na.rm=TRUE)[4]&!is.na(DAT$SQ)] <- 1
  }
  if(length(unique(DAT$SQ[DAT$Target==Targets[i]]))!=length(STDS2)) {
    ToWrite <- paste0("These standards not included in ",
                      Targets[i],
                      " standard curve regression for copy estimate calculations, because they detected below 100%: ",
                      paste(setdiff(unique(DAT$SQ[DAT$Target==Targets[i]]),
                                    STDS2),
                            collapse=", "),
                      "\n\n")
    write(ToWrite,
  ...
  assign(paste0("curve",i),
         lm(Cq~log10(SQ),
            data=DAT[DAT$Target==Targets[i]&DAT$Mod==1,]))
...
## Summarize the data:
DAT2 <- data.frame(Standards=Standards,
                   Target=Target,
                   Reps=NA,
                   Detects=NA,
                   Cq.mean=NA,
...
for(i in 1:nrow(DAT2)) {
  DAT2$Reps[i] <- sum(DAT$SQ==DAT2$Standards[i]&DAT$Target==DAT2$Target[i],
                      na.rm=TRUE)
  DAT2$Detects[i] <- sum(!is.na(DAT$Cq)&DAT$SQ==DAT2$Standards[i]&DAT$Target==DAT2$Target[i],
                         na.rm=TRUE)
  DAT2$Cq.mean[i] <- mean(DAT$Cq[DAT$SQ==DAT2$Standards[i]&DAT$Target==DAT2$Target[i]],
                          na.rm=TRUE)
  DAT2$Cq.sd[i] <- sd(DAT$Cq[DAT$SQ==DAT2$Standards[i]&DAT$Target==DAT2$Target[i]],
                      na.rm=TRUE)
  DAT2$Copy.CV[i] <- sd(DAT$Copy.Estimate[DAT$SQ==DAT2$Standards[i]&DAT$Target==DAT2$Target[i]],
                        na.rm=TRUE)/mean(DAT$Copy.Estimate[DAT$SQ==DAT2$Standards[i]&DAT$Target==DAT2$Target[i]],
                                         na.rm=TRUE)
...
write("Data Summary:",
      file="Analysis Log.txt",
      append=TRUE)
suppressWarnings(write.table(DAT2,
                             file="Analysis Log.txt",
                             append=TRUE,sep="\t",
                             eol="\n",
                             row.names=FALSE,
...
for(i in 1:length(Targets)) {
  ...
  ToWrite <- paste0("For ",
                    Targets[i],
                    ", the lowest standard with 95% or greater detection is: ",A," copies/reaction.")
  ToWrite2 <- ""
...
for(i in 1:length(Targets)) {
  ...
  if(sum(DAT2$Rate[DAT2$Target==Targets[i]]!=1&DAT2$Rate[DAT2$Target==Targets[i]]!=0)==0) {
    ToWrite <- paste0("WARNING: For ",
                      Targets[i],
                      ", all standards detected fully or failed fully.  Therefore, the LoD model will not converge.")
  ...
  if(sum(DAT2$Rate[DAT2$Target==Targets[i]]!=1&DAT2$Rate[DAT2$Target==Targets[i]]!=0)==1) {
    ToWrite <- paste0("WARNING: For ",
                      Targets[i],
                      ", only 1 standard detected in the informative range (not 0% and not 100%).  Therefore, the LoD model results will be less reliable.")
    write(paste0(ToWrite,
                 "\n\n"),
          file="Analysis Log.txt",
          append=TRUE)
  ...
  ## Define probit model:
assign(paste0("LOD.mod",  i),
       glm(Detect~SQ,
           data=DAT[DAT$Target==Targets[i],],
...
LOD.list <- c(LOD.list,
  ...
  if(LOQ.FCT=="Best") {
    ...
    tryCatch({ #skip if model cannot be determined.
      ...
      e
      cat("ERROR: decay LOQ model cannot be defined for ",
    ...
    tryCatch({ #skip if model cannot be determined.
      ...
      e
      cat("ERROR: linear LOQ model cannot be defined for ",
          as.character(Targets[i]),
          sep="")
    })
    tryCatch({ #skip if model cannot be determined.
      LOQ3 <- lm(Cq.CV~poly(log10(Standards),2),
                 data=DAT2[DAT2$Target==Targets[i],])
    }, error=function(e) {
      e
      cat("ERROR: 2nd polynomial LOQ model cannot be defined for ",
          as.character(Targets[i]),
          sep="")
    })
    tryCatch({ #skip if model cannot be determined.
      LOQ4 <- lm(Cq.CV~poly(log10(Standards),3),
                 data=DAT2[DAT2$Target==Targets[i],])
    }, error=function(e) {
      e
      cat("ERROR: 3rd polynomial LOQ model cannot be defined for ",
          as.character(Targets[i]),
          sep="")
    })
    tryCatch({ #skip if model cannot be determined.
      LOQ5 <- lm(Cq.CV~poly(log10(Standards),4),
                 data=DAT2[DAT2$Target==Targets[i],])
    }, error=function(e) {
      e
      cat("ERROR: 4th polynomial LOQ model cannot be defined for ",
          as.character(Targets[i]),sep="")
    })
    tryCatch({ #skip if model cannot be determined.
      LOQ6 <- lm(Cq.CV~poly(log10(Standards),5),
                 data=DAT2[DAT2$Target==Targets[i],])
    }, error=function(e) {
      e
      cat("ERROR: 5th polynomial LOQ model cannot be defined for ",
          as.character(Targets[i]),
          sep="")
    })
    tryCatch({ #skip if model cannot be determined.
      LOQ7 <- lm(Cq.CV~poly(log10(Standards),6),
                 data=DAT2[DAT2$Target==Targets[i],])
    }, error=function(e) {
      e
      cat("ERROR: 6th polynomial LOQ model cannot be defined for ",
          as.character(Targets[i]),
          sep="")
    })
    ...
    A <- sapply(c("LOQ1",
                ...
                exists)
    B <- names(A)[A==TRUE]
    ## If at least 1 LOQ model was determined, select the one with the lowest residual standard error:
    if(length(B)>0) {
      LOQ.res <- rep(NA,
                     length(B))
      for(j in 1:length(B)) {
      ...
      C <- which(LOQ.res==min(LOQ.res,
                              na.rm=TRUE))
      assign(paste0("LOQ.mod",i),
             get(B[C]))
      LOQ.list <- c(LOQ.list,
                    paste0("LOQ.mod",i))
    }
  ...
  if(LOQ.FCT=="Decay") {
    tryCatch({ #skip if model cannot be determined.
      assign(paste0("LOQ.mod",i),
             nls(Cq.CV~SSasymp(log10(Standards),
                               Asym,R0,lrc),
                                     data=DAT2[DAT2$Target==Targets[i],]))
  ...
  if(is.list(LOD.FCT)==TRUE) {
    tryCatch({ #skip if model cannot be determined.
      assign(paste0("LOD.mod2",i),
             drm(Detect~SQ,data=DAT[DAT$Target==Targets[i],],
                                      fct=LOD.FCT))
      LOD.list2 <- c(LOD.list2,paste0("LOD.mod2",i))
      LOD.list3 <- c(LOD.list3,LOD.FCT$name)
    }, error=function(e) {
      e
      cat("ERROR: LOD model cannot be defined for ",
          as.character(Targets[i]),sep="")
    })
  }
  ...
  if(is.character(LOD.FCT)) {
    if(LOD.FCT=="Best") {
      tryCatch({ #skip if model cannot be determined.
        ...
        LOD.mod <- drm(Detect~SQ,
                       data=TEMP.DAT,
                       fct=W2.4())
        ## Test all available models and select the best one:
        LOD.FCT2 <- row.names(mselect(LOD.mod,
                                      LOD.FCTS))[1]
        LOD.FCT3 <- getMeanFunctions(fname=LOD.FCT2)
        assign(paste0("LOD.mod2",i),
               drm(Detect~SQ,data=DAT[DAT$Target==Targets[i],],
                   fct=LOD.FCT3[[1]]))
        LOD.list2 <- c(LOD.list2,
                       paste0("LOD.mod2",i))
        LOD.list3 <- c(LOD.list3,
                       LOD.FCT2)
      }, error=function(e) {
        e
        cat("ERROR: LOD model cannot be defined for ",
  ...
  if(!is.na(LOD.list2[i+1])) {
    DAT3$LOD[i] <- ED(get(LOD.list2[i+1]),
                      0.95,
                      type="absolute")[1]
    DAT3$rep2.LOD[i] <- ED(get(LOD.list2[i+1]),
                           1-sqrt(0.05),
                           type="absolute")[1]
    DAT3$rep3.LOD[i] <- ED(get(LOD.list2[i+1]),
                           1-0.05^(1/3),
                           type="absolute")[1]
    DAT3$rep4.LOD[i] <- ED(get(LOD.list2[i+1]),
                           1-0.05^0.25,
                           type="absolute")[1]
    DAT3$rep5.LOD[i] <- ED(get(LOD.list2[i+1]),
                           1-0.05^0.2,
                           type="absolute")[1]
    DAT3$rep8.LOD[i] <- ED(get(LOD.list2[i+1]),
                           1-0.05^0.125,
                           type="absolute")[1]
...
DAT3$rep8.LOD[i] <- (qnorm(0.125)-coef(get(LOD.list[i+1]))[1])/coef(get(LOD.list[i+1]))[2]
  ...
  if(!is.na(LOQ.list[i+1])) {
    ...
    newData$Cq.CV <- predict(get(LOQ.list[i+1]), 
                             newData)
    ...
    if(as.character(get(LOQ.list[i+1])$call)[1]=="nls") {
      ...
      if(min(newData$Cq.CV[newData$Standards<=A])>LOQ.Threshold) {
        ...
        ToWrite <- paste0("Note: All standards tested for ",
                          Targets[i],
                          " yielded higher Cq.CV values than the user-defined CV threshold of ",
                          LOQ.Threshold,
                          ". The CV threshold has been adjusted to ",
    ...
    if(as.character(get(LOQ.list[i+1])$call)[1]=="lm") {
      ...
      if(grepl("poly",
        ...
        if(min(DAT2$Cq.CV[DAT2$Target==Targets[i]],
               na.rm=TRUE)>LOQ.Threshold) {
          B <- min(DAT2$Cq.CV[DAT2$Target==Targets[i]],
                   na.rm=TRUE)*1.5
          ## Make a note of the adjusted threshold in the analysis log:
          ToWrite <- paste0("Note: All standards tested for ",
                            Targets[i],
...
## Plot Cycle Threshold value vs Standard Concentration standard curves:
...
for(i in 1:length(Targets)) {
  ggOut <- ggplot(data=DAT[DAT$Target==Targets[i]&is.na(DAT$SQ)==FALSE,], 
    ...
    scale_shape_manual("", 
                       values=c(3,10), 
                       guide=FALSE) + #Change shape of excluded/modeled point markers
    scale_size_manual("", 
                      values=c(1,1)) +
    scale_x_log10() +
    scale_color_manual("", 
                       values=c("red", 
                                "black")) +
    xlab("Concentration (copies/µL)") +
    ylab("Cycle Threshold Value") +
    geom_abline(intercept=coef(get(curve.list[i+1]))[1], 
                slope=coef(get(curve.list[i+1]))[2]) +
    geom_vline(xintercept=DAT3$LOD[i], 
               colour="blue") +
    geom_vline(xintercept=DAT3$LOQ[i], 
               linetype=2) +
    annotate("text", 
             y=max(DAT$Cq[DAT$Target==Targets[i]], 
                   na.rm=TRUE)*0.95, 
             color="blue", 
             x=DAT3$LOD[i]*0.7, 
             angle=90, 
    ...
    annotate("text", 
             ...
             angle=90, 
             label="LOQ") +
    theme_bw() + theme(legend.justification=c(0.8,0.8), 
    ...
    annotate("text", 
             ...
             xmin=0.1,
             label=paste0("R² = ", 
                          signif(DAT3$R.squared[i], digits = 4),
                          "\ny = ", 
                          signif(DAT3$Slope[i], digits = 4),
                          "x + ", 
                          round(DAT3$Intercept[i], digits = 3)),
             hjust="left", 
             vjust="bottom", 
             size=4)
  print(ggOut)
...
for(i in 1:length(Targets)) {
  if(!is.na(LOD.list2[i+1])) {
    DAT4 <- rbind(ED(get(LOD.list2[i+1]),
                     0.95,
                     interval="delta",
                     type="absolute"),
                  ED(get(LOD.list2[i+1]),
                     1-sqrt(0.05),
                     interval="delta",
                     type="absolute"),
                  ED(get(LOD.list2[i+1]),
    ...
    plot(get(LOD.list2[i+1]),
         main=paste0("LoD Plot for: ",
                     Targets[i]),
    ...
    LODS2 <- c("Limit of Detection",
               ...
               "8 Replicates LoD")
    if(LODS<6) {
      LODS2[(LODS+1):6] <- gsub("licates LoD",
                                "s: Insufficient Data",
                                LODS2[(LODS+1):6])
    }
    points(x=DAT4[1:LODS,1],
           y=YS[1:LODS],
           pch=PNTS,
           col=COLS,
           cex=1.2)
    for(j in 1:LODS) {
      lines(x=DAT4[j,3:4],
            ...
            col=COLS[j],
            lwd=2)
      lines(x=rep(DAT4[j,3],2),
            y=c(YS[j]-0.02,
                YS[j]+0.02),
            lwd=2,
            col=COLS[j])
      lines(x=rep(DAT4[j,4],2),
            y=c(YS[j]-0.02,
                YS[j]+0.02),
            lwd=2,col=COLS[j])
    }
...
for(i in 1:length(Targets)) {
  if(is.na(LOQ.list[i+1])==FALSE) {
    ...
    PDAT <- data.frame(x=c(min(DAT2$Standards[DAT2$Target==Targets[i]],
                               na.rm=TRUE),
                           min(DAT2$Standards[DAT2$Target==Targets[i]],
                               na.rm=TRUE),
                           DAT3$LOQ[DAT3$Assay==Targets[i]],
                           DAT3$LOQ[DAT3$Assay==Targets[i]]),
                       y=c(min(c(DAT2$Cq.CV[DAT2$Target==Targets[i]],
                                 newData$Cq.CV[newData$Standards<=max(DAT2$Standards[DAT2$Target==Targets[i]])&newData$Standards>=min(DAT2$Standards[DAT2$Target==Targets[i]])]),
                               na.rm=TRUE)*0.9,
                           newData$Cq.CV[newData$Standards==DAT3$LOQ[DAT3$Assay==Targets[i]]],
                           newData$Cq.CV[newData$Standards==DAT3$LOQ[DAT3$Assay==Targets[i]]],
                           min(c(DAT2$Cq.CV[DAT2$Target==Targets[i]],
                                 newData$Cq.CV[newData$Standards<=max(DAT2$Standards[DAT2$Target==Targets[i]])&newData$Standards>=min(DAT2$Standards[DAT2$Target==Targets[i]])]),
                               na.rm=TRUE)*0.9))
  ...
  Decay.Plot <- ggplot(DAT2[DAT2$Target==Targets[i],], 
                       aes(x= Standards, y = Cq.CV)) +
    geom_point(size=2) +
